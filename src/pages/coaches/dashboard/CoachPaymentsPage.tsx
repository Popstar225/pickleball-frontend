...
import {
  DollarSign, CreditCard, Calendar, Clock, AlertCircle,
  Receipt, Star, Loader2, FileText, RefreshCw,
} from 'lucide-react';
...
const STATUS_CFG = {
  completed: { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400', dot: 'bg-emerald-400',              label: 'Completado' },
  pending:   { bg: 'bg-amber-500/10',   border: 'border-amber-500/20',   text: 'text-amber-400',   dot: 'bg-amber-400 animate-pulse',   label: 'Pendiente'  },
  failed:    { bg: 'bg-red-500/10',     border: 'border-red-500/20',     text: 'text-red-400',     dot: 'bg-red-400',                   label: 'Fallido'    },
} as const;
...
// ─── Atoms ────────────────────────────────────────────────────────────────────
function SectionCard({ children, accentColor = 'from-[#ace600]/30' }: {
  children: React.ReactNode; accentColor?: string;
}) {
  return (
    <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl overflow-hidden">
      <div className={cn('h-0.5 bg-gradient-to-r via-transparent to-transparent', accentColor)} />
...
export default function CoachPaymentsPage() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-24 gap-3">
      ...
      <p className="text-xs text-white/20">Cargando datos de pago…</p>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <h1 className="text-[22px] font-black text-white tracking-tight">Pagos y Facturación</h1>
      ...
      <div className={cn(
        ...
        isActive ? 'border-[#ace600]/30 shadow-[0_0_20px_rgba(172,230,0,0.06)]' : 'border-white/[0.07]',
      )}>
        <div className={cn('h-0.5 bg-gradient-to-r via-transparent to-transparent', isActive ? 'from-[#ace600]/50' : 'from-white/[0.06]')} />
        <div className="p-5">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              <div className={cn('w-9 h-9 rounded-xl border flex items-center justify-center shrink-0',
                isActive ? 'bg-[#ace600]/10 border-[#ace600]/20' : 'bg-white/[0.04] border-white/[0.08]')}>
            ...
            <span className={cn(
              ...
              <span className={cn('w-1.5 h-1.5 rounded-full',
                isActive ? 'bg-emerald-400' : membership?.is_expired ? 'bg-red-400' : 'bg-white/20')} />
              {isActive ? 'Activa' : membership?.is_expired ? 'Vencida' : 'Inactiva'}
            </span>
          </div>

          <p className="text-3xl font-black text-white mb-1">
            ${(membership?.annual_fee || 800).toLocaleString()}
            <span className="text-base text-white/25 font-bold"> MXN / año</span>
          </p>
          {isActive && membership?.membership_expires_at && (
            <p className="text-[11px] text-white/30 mb-4 flex items-center gap-1.5">
          ...
          <button onClick={() => setShowMembershipModal(true)}
            className={cn(
              'w-full h-9 mt-3 rounded-xl text-xs font-black inline-flex items-center justify-center gap-1.5 transition-all',
              isActive
      ...
      {/* ── Payment History ───────────────────────────────────────────────────── */}
      <SectionCard accentColor="from-violet-400/35">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-white/25 mt-0.5">Todos tus pagos y transacciones</p>
          </div>
          {payments.length > 0 && (
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-b border-white/[0.05] hover:bg-transparent">
                  {['Descripción', 'Tipo', 'Monto', 'Estado', 'Fecha', ''].map(h => (
                    <TableHead key={h} className="text-[10px] font-black uppercase tracking-widest text-white/20 h-10 px-4">{h}</TableHead>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
