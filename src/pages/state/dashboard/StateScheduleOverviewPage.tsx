/**
 * State Schedule Overview Page
 * Route: /state/dashboard/schedule
 */
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function StateScheduleOverviewPage() {
    ...
    const load = async () => {
      try {
        ...
        const url = organizerId
          ...
          : '/tournaments?tournament_type=state&limit=50';
      ...
      {tournaments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4 bg-[#0d1117] border border-white/[0.07] rounded-2xl">
          ...
          <div className="text-center">
            ...
            <button
              onClick={() => navigate('/state/dashboard/tournaments')}
              ...
              <div className="px-5 py-4 border-b border-white/[0.05] flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/30 mt-0.5 flex items-center gap-1">
                    ...
                    <span className="text-white/25">{tournament.events?.length ?? 0} evento{tournament.events?.length !== 1 ? 's' : ''}</span>
                  </p>
                </div>
              ...
              <div className="divide-y divide-white/[0.04]">
                {(tournament.events ?? []).map((ev, idx) => (
                  <div key={ev.id}
                    ...
                    <button
                      onClick={() =>
                        navigate(`/state/dashboard/tournaments/${tournament.id}/events/${ev.id}/schedule`)
                      }
