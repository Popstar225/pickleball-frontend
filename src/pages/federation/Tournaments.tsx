...
// // Tournament type - transformed from API response
...
// // ─── Helper function to transform API data to tournament format ───
// const transformApiTournament = (tournament: any): Tournament => {
//   return {
//     id: tournament.id,
...
//     endDate: tournament.end_date || new Date().toISOString(),
//     registrationDeadline: tournament.registration_deadline || new Date().toISOString(),
//     location: tournament.city || 'Ubicación no disponible',
//     state: tournament.state || 'México',
//     venue: tournament.venue_name || 'Venue',
//     categories: tournament.category ? [tournament.category] : ['Categoría no especificada'],
//     format: tournament.format || 'Doble eliminación',
//     maxParticipants: tournament.max_participants || 100,
//     currentParticipants: tournament.current_participants || 0,
//     entryFee: tournament.entry_fee ? `$${tournament.entry_fee}` : 'Gratis',
//     prizePool: tournament.prize_pool ? `$${tournament.prize_pool}` : null,
//     image: 'https://images.unsplash.com/photo-1461896836934-ffe607ba8211?w=800&q=80', // Default image
//     status: tournament.status || 'published',
//     createdBy:
//       tournament.organizer_type === 'admin' ? 'federation' : tournament.organizer_type || 'club',
//     creatorName: tournament.organizer_name || 'Organizador',
//     validatedBy: tournament.validated_by || null,
//     validatedAt: tournament.validated_at || null,
//     featured: tournament.is_featured || false,
//   };
// };
...
//   state: { label: 'Comité Estatal', icon: Award },
//   club: { label: 'Club', icon: Building2 },
//   partner: { label: 'Socio', icon: Sparkles },
//   admin: { label: 'Federación', icon: Shield },
// };
...
//   // Transform API tournaments to local format
//   const tournaments = useMemo(() => rawTournaments.map(transformApiTournament), [rawTournaments]);

//   const states = useMemo(() => [...new Set(tournaments.map((t) => t.state))], [tournaments]);
...
//             <button
//               onClick={() => dispatch(fetchTournaments({ page: 1, limit: 100 }))}
//               className="bg-primary text-primary-foreground px-6 py-2 rounded-lg font-semibold hover:bg-lime-dark transition-colors"
//             >
//               Reintentar
//             </button>
//           </div>
