...
import playerActionImage from '@/assets/images/blogs/image7.png'; // Action shot
    ...
    try {
      ...
      if (loginResult.type === 'auth/loginUser/fulfilled') {
        ...
        if (apiResponse?.data?.user && apiResponse?.data?.tokens) {
          ...
          toast.error(t('auth.login.toast_error_server'));
        }
      } else if (loginResult.type === 'auth/loginUser/rejected') {
        const errorMessage = loginResult?.payload || t('auth.login.toast_error');
        toast.error(errorMessage);
      }
    } catch (err) {
      toast.error(t('auth.login.toast_error_unexpected'));
