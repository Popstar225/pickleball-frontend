...
import ScoreCorrectionApprovalPage from './Tournament/ScoreCorrectionApprovalPage';
...
export default function ClubScoreCorrectionsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await api.get<any>('/tournaments?tournament_type=local&myTournaments=true&limit=50');
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-white/25">Aprueba o rechaza solicitudes de corrección de marcador</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-white/25">No tienes torneos activos con correcciones pendientes</p>
        </div>
      ) : (
        <>
