...
function Field({ label, value, onChange, disabled, type = 'text', textarea, rows = 3, placeholder }: {
  ...
  return (
    <div>
      ...
      {textarea
        ? <textarea className={`${inputCls} resize-y`} style={{ minHeight: rows * 28 }} value={value} onChange={e => onChange(e.target.value)} disabled={disabled} placeholder={placeholder} />
        : <input className={inputCls} type={type} value={value} onChange={e => onChange(e.target.value)} disabled={disabled} placeholder={placeholder} />
      }
    </div>
  );
}

function Dropdown({ label, value, onChange, disabled, options, placeholder = 'Seleccionar...' }: {
      ...
      <Select value={value} onValueChange={onChange} disabled={disabled}>
        ...
        <SelectContent className="bg-[#161c25] border border-white/[0.08] rounded-xl shadow-2xl">
          {options.map(opt => {
            ...
            return <SelectItem key={v} value={v} className="text-white/70 focus:bg-white/[0.06] focus:text-white">{l}</SelectItem>;
          })}
        </SelectContent>
      </Select>
    </div>
...
function SaveBar({ visible, onSave, onCancel, loading }: {
  ...
  return (
    <div className="sticky bottom-4 z-20 mt-4 transition-all duration-200"
      ...
      <div className="flex items-center justify-between gap-4 px-5 py-3.5 rounded-2xl border border-white/[0.08] shadow-2xl"
...
function toForm(p: ClubProfile): FormData {
  return {
    id: p.id ?? '', name: p.name ?? '', clubType: p.clubType ?? '', description: p.description ?? '',
    contactPerson: p.contactPerson ?? '', contactEmail: p.contactEmail ?? '', contactPhone: p.contactPhone ?? '',
    contactWhatsapp: p.contactWhatsapp ?? '', state: p.state ?? '', city: p.city ?? '', address: p.address ?? '',
    latitude: p.latitude ?? '', longitude: p.longitude ?? '', foundedDate: p.foundedDate ?? '',
    memberCount: p.memberCount ?? 0, maxMembers: p.maxMembers ?? 0, hasCourts: p.hasCourts ?? false,
    courtCount: p.courtCount ?? 0, courtTypes: p.courtTypes ?? [], offersTraining: p.offersTraining ?? false,
    offersTournaments: p.offersTournaments ?? false, offersEquipment: p.offersEquipment ?? false,
    membershipStatus: p.membershipStatus ?? 'active', membershipExpiresAt: p.membershipExpiresAt ?? '',
    subscriptionPlan: p.subscriptionPlan ?? 'basic', membershipFee: p.membershipFee ?? '',
    courtRentalFee: p.courtRentalFee ?? '', logoUrl: p.logoUrl ?? '', bannerImage: p.bannerImage ?? '',
    photos: p.photos ?? [], website: p.website ?? '', socialMedia: p.socialMedia ?? {},
    operatingHours: p.operatingHours ?? {}, availability: p.availability ?? { court_booking:false, drop_in:false, leagues:false, tournaments:false },
    clubRules: p.clubRules ?? '', dressCode: p.dressCode ?? '', totalTournaments: p.totalTournaments ?? 0,
    totalMatches: p.totalMatches ?? 0, averageRating: p.averageRating ?? '', reviewCount: p.reviewCount ?? 0,
    isActive: p.isActive ?? false, isVerified: p.isVerified ?? false, isFeatured: p.isFeatured ?? false,
    settings: p.settings ?? { allow_guest_play:false, max_guest_visits:0, auto_approve_members:false },
    notes: p.notes ?? '', owner: p.owner ?? { id:'', name:'', email:'', profilePhoto:null },
  };
}
...
export default function ClubAccountPage() {
    ...
    try {
      if (isNewClub && profileError) {
        await dispatch(createClub({
          ...
          contact_whatsapp: formData.contactWhatsapp, state: formData.state, city: formData.city,
          address: formData.address, latitude: formData.latitude ? Number(formData.latitude) : undefined,
          longitude: formData.longitude ? Number(formData.longitude) : undefined,
          founded_date: formData.foundedDate, max_members: formData.maxMembers,
          has_courts: formData.hasCourts, court_count: formData.courtCount, court_types: finalCourtTypes,
          offers_training: formData.offersTraining, offers_tournaments: formData.offersTournaments,
          ...
          website: formData.website, social_media: formData.socialMedia,
          operating_hours: hours ?? formData.operatingHours, club_rules: formData.clubRules,
  ...
  const statusCfg = formData.membershipStatus === 'active'
    ...
    ? { bg:'bg-red-500/10', text:'text-red-400', border:'border-red-500/20', label:'Expirado' }
    : { bg:'bg-amber-500/10', text:'text-amber-400', border:'border-amber-500/20', label:'Pendiente' };

  const planLabel = SUBSCRIPTION_PLAN_OPTIONS.find(o => o.value === formData.subscriptionPlan)?.label ?? formData.subscriptionPlan;
  const memberPct = formData.maxMembers > 0 ? Math.min((formData.memberCount / formData.maxMembers) * 100, 100) : 0;
      ...
      <div className="max-w-5xl mx-auto flex flex-col gap-6">
        ...
        <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl overflow-hidden">
          ...
          <div className="relative h-24 overflow-hidden"
            style={{ background: formData.bannerImage ? `url(${formData.bannerImage}) center/cover no-repeat` : 'linear-gradient(135deg,#0f1e3a 0%,#0d1117 100%)' }}>
          ...
          <div className="px-6 pb-5 -mt-8 relative flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <div className="flex items-end gap-4">
              <div className="relative shrink-0">
                <div className="w-16 h-16 rounded-2xl border-2 border-[#0d1117] flex items-center justify-center text-xl font-bold overflow-hidden shadow-xl"
                  style={{ background: formData.logoUrl ? `url(${formData.logoUrl}) center/cover no-repeat` : `linear-gradient(135deg,#1a2744,#0f1e3a)`, color: A }}>
                  {!formData.logoUrl && initials}
                </div>
                <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#0d1117]"
              ...
              <div className="pb-0.5">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  {formData.isVerified && (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider bg-[#ace600]/10 border border-[#ace600]/20" style={{ color: A }}>
                      Verificado
                    </span>
                  )}
                  {formData.isFeatured && (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent className="bg-[#0d1117] border border-white/[0.08] rounded-2xl p-0 shadow-2xl overflow-hidden max-w-sm">
                  <div className="p-6">
                    ...
                    <AlertDialogHeader>
                      <AlertDialogTitle className="text-base font-bold text-white">¿Eliminar este club?</AlertDialogTitle>
                      <AlertDialogDescription className="text-sm text-white/35 leading-relaxed mt-1">
                        Esta acción no se puede deshacer. Se eliminarán permanentemente el club y todos sus datos.
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl p-4">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/25">Miembros</p>
            </div>
            <div className="flex items-end gap-1.5">
              <span className="text-2xl font-bold leading-none" style={{ color: A }}>{formData.memberCount}</span>
              {formData.maxMembers > 0 && <span className="text-xs text-white/25 mb-0.5">/ {formData.maxMembers}</span>}
            </div>
            {formData.maxMembers > 0 && (
              <div className="mt-2.5 h-1 rounded-full bg-white/[0.06] overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${memberPct}%`, background: A }} />
              </div>
            )}
          </div>
          ...
          <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl p-4">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/25">Torneos</p>
            </div>
            <span className="text-2xl font-bold leading-none" style={{ color: A }}>{formData.totalTournaments}</span>
          ...
          <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl p-4">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/25">Rating</p>
            </div>
            <div className="flex items-end gap-1.5">
              <span className="text-2xl font-bold leading-none" style={{ color: A }}>{formData.averageRating || '—'}</span>
              {formData.averageRating && <span className="text-xs text-white/25 mb-0.5">/ 5.0</span>}
            </div>
          ...
          <div className="bg-[#0d1117] border border-white/[0.07] rounded-2xl p-4">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/25">Suscripción</p>
            </div>
            <span className={`inline-flex text-xs font-bold px-2.5 py-1 rounded-lg border ${statusCfg.bg} ${statusCfg.text} ${statusCfg.border}`}>
              {statusCfg.label}
            </span>
        ...
        <Tabs value={activeTab} onValueChange={v => setActiveTab(v as TabValue)}>
          <TabsList className="flex h-auto w-full overflow-x-auto gap-1 bg-[#0d1117] border border-white/[0.07] rounded-2xl p-1.5 scrollbar-none">
            {TAB_CONFIG.map(({ value, label, icon: Icon }) => (
              <TabsTrigger key={value} value={value}
                className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-semibold whitespace-nowrap transition-all text-white/30 hover:text-white/55 hover:bg-white/[0.04] data-[state=active]:bg-[#ace600] data-[state=active]:text-black data-[state=active]:font-bold data-[state=active]:shadow-[0_0_10px_rgba(172,230,0,0.18)]"
              >
          ...
          <TabsContent value="identity" className="mt-4">
            <SectionCard>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Nombre del Club" value={formData.name} onChange={v => set('name', v)} disabled={isDisabled} placeholder="Nombre oficial" />
                <Dropdown label="Tipo de Club" value={formData.clubType} onChange={v => set('clubType', v)} disabled={isDisabled} options={CLUB_TYPES_OPTIONS} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Sitio Web" value={formData.website} onChange={v => set('website', v)} disabled={isDisabled} placeholder="https://miclub.com" />
                <Field label="Fecha de Fundación" value={formData.foundedDate?.split('T')[0] || ''} onChange={v => set('foundedDate', v)} disabled={isDisabled} type="date" />
              </div>
              <Field label="Descripción" value={formData.description} onChange={v => set('description', v)} disabled={isDisabled} textarea rows={4} placeholder="Describe tu club…" />
              <Field label="Notas Internas" value={formData.notes} onChange={v => set('notes', v)} disabled={isDisabled} textarea rows={2} placeholder="Notas de administración…" />
            </SectionCard>
          </TabsContent>
          ...
          <TabsContent value="contact" className="mt-4">
            <SectionCard>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Persona de Contacto" value={formData.contactPerson} onChange={v => set('contactPerson', v)} disabled={isDisabled} placeholder="Nombre completo" />
                <Field label="Correo Electrónico" value={formData.contactEmail} onChange={v => set('contactEmail', v)} disabled={isDisabled} type="email" placeholder="correo@club.com" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Teléfono" value={formData.contactPhone} onChange={v => set('contactPhone', v)} disabled={isDisabled} placeholder="+52 55 0000 0000" />
                <Field label="WhatsApp" value={formData.contactWhatsapp} onChange={v => set('contactWhatsapp', v)} disabled={isDisabled} placeholder="+52 55 0000 0000" />
              </div>
            </SectionCard>
          </TabsContent>
          ...
          <TabsContent value="location" className="mt-4">
            <SectionCard>
              <Field label="Dirección" value={formData.address} onChange={v => set('address', v)} disabled={isDisabled} placeholder="Calle, número, colonia" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Ciudad" value={formData.city} onChange={v => set('city', v)} disabled={isDisabled} placeholder="Ciudad" />
                <div>
                  <label className={labelCls}>Estado</label>
                  <StateAutocomplete value={formData.state} onChange={v => set('state', v)} disabled={isDisabled} placeholder="Buscar estado…" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Latitud" value={formData.latitude} onChange={v => set('latitude', v)} disabled={isDisabled} placeholder="19.4326" />
                <Field label="Longitud" value={formData.longitude} onChange={v => set('longitude', v)} disabled={isDisabled} placeholder="-99.1332" />
              </div>
            </SectionCard>
          </TabsContent>
          ...
          <TabsContent value="courts" className="mt-4">
            <SectionCard>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Field label="Miembros Actuales" value={formData.memberCount.toString()} onChange={v => set('memberCount', Number(v))} disabled={isDisabled} type="number" />
                <Field label="Máximo de Miembros" value={formData.maxMembers.toString()} onChange={v => set('maxMembers', Number(v))} disabled={isDisabled} type="number" />
                <Field label="Cantidad de Canchas" value={formData.courtCount.toString()} onChange={v => set('courtCount', Number(v))} disabled={isDisabled} type="number" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {/* Court types */}
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    {formData.courtTypes.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {formData.courtTypes.map((ct, i) => (
                          <span key={i} className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg border"
                            style={{ background:'rgba(172,230,0,0.08)', border:'1px solid rgba(172,230,0,0.2)', color:A }}>
                            {ct}
                            {!isDisabled && (
                              <button type="button" onClick={() => set('courtTypes', formData.courtTypes.filter((_, j) => j !== i))} className="hover:opacity-60 transition-opacity">
                                <X className="w-3 h-3" />
                              </button>
                            )}
                          </span>
                    ...
                    <div className="flex gap-2">
                      <input className={inputCls} type="text" value={courtTypesRaw} placeholder="Ej. Polvo de ladrillo" disabled={isDisabled}
                        onChange={e => setCourtTypesRaw(e.target.value)}
                        onKeyDown={e => { if (e.key === 'Enter' && courtTypesRaw.trim()) { e.preventDefault(); const t = courtTypesRaw.trim(); if (!formData.courtTypes.includes(t)) { set('courtTypes', [...formData.courtTypes, t]); setCourtTypesRaw(''); } } }} />
                      <button type="button" disabled={isDisabled || !courtTypesRaw.trim()}
                        onClick={() => { const t = courtTypesRaw.trim(); if (t && !formData.courtTypes.includes(t)) { set('courtTypes', [...formData.courtTypes, t]); setCourtTypesRaw(''); } }}
                        className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all disabled:opacity-30"
                        style={{ background: !isDisabled && courtTypesRaw.trim() ? A : 'rgba(255,255,255,0.06)', color: !isDisabled && courtTypesRaw.trim() ? '#080c14' : 'rgba(255,255,255,0.25)' }}>
              ...
              <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl px-4">
                <Toggle label="Canchas Propias" description="El club cuenta con instalaciones propias" checked={formData.hasCourts} onChange={v => set('hasCourts', v)} disabled={isDisabled} />
                <Toggle label="Ofrece Entrenamiento" description="Clases y entrenamiento disponibles" checked={formData.offersTraining} onChange={v => set('offersTraining', v)} disabled={isDisabled} />
                <Toggle label="Organiza Torneos" description="El club organiza torneos propios" checked={formData.offersTournaments} onChange={v => set('offersTournaments', v)} disabled={isDisabled} />
                <Toggle label="Vende Equipamiento" description="Tienda de equipos y accesorios" checked={formData.offersEquipment} onChange={v => set('offersEquipment', v)} disabled={isDisabled} />
              </div>
            </SectionCard>
          </TabsContent>
          ...
          <TabsContent value="availability" className="mt-4">
            <SectionCard>
              <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl px-4">
                <Toggle label="Reserva de Canchas" description="Los miembros pueden reservar canchas online" checked={formData.availability.court_booking} onChange={v => setAvailability('court_booking', v)} disabled={isDisabled} />
                <Toggle label="Drop-In" description="Acceso libre sin reserva previa" checked={formData.availability.drop_in} onChange={v => setAvailability('drop_in', v)} disabled={isDisabled} />
          ...
          <TabsContent value="membership" className="mt-4">
            <SectionCard>
              ...
              <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl px-4">
                <Toggle label="Club Activo" description="El club está operando actualmente" checked={formData.isActive} onChange={v => set('isActive', v)} disabled={true} />
                <Toggle label="Club Verificado" description="Verificación oficial completada" checked={formData.isVerified} onChange={v => set('isVerified', v)} disabled={true} />
                <Toggle label="Club Destacado" description="Aparece como destacado en búsquedas" checked={formData.isFeatured} onChange={v => set('isFeatured', v)} disabled={true} />
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <Field label="Total de Torneos" value={formData.totalTournaments.toString()} onChange={v => set('totalTournaments', Number(v))} disabled={true} type="number" />
                <Field label="Total de Partidos" value={formData.totalMatches.toString()} onChange={v => set('totalMatches', Number(v))} disabled={true} type="number" />
                <Field label="Rating Promedio" value={formData.averageRating} onChange={v => set('averageRating', v)} disabled={true} placeholder="4.5" />
                <Field label="Reseñas" value={formData.reviewCount.toString()} onChange={v => set('reviewCount', Number(v))} disabled={true} type="number" />
              </div>
            </SectionCard>
          </TabsContent>
          ...
          <TabsContent value="rules" className="mt-4">
            <SectionCard>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Reglas del Club" value={formData.clubRules} onChange={v => set('clubRules', v)} disabled={isDisabled} textarea rows={5} placeholder="Normas de convivencia…" />
                <Field label="Código de Vestimenta" value={formData.dressCode} onChange={v => set('dressCode', v)} disabled={isDisabled} textarea rows={5} placeholder="Indumentaria requerida…" />
              </div>
              <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl px-4">
                <Toggle label="Permitir Invitados" description="Los miembros pueden traer invitados" checked={formData?.settings?.allow_guest_play} onChange={v => setSettings('allow_guest_play', v)} disabled={isDisabled} />
                <Toggle label="Auto-aprobar Miembros" description="Nuevos miembros aprobados automáticamente" checked={formData?.settings?.auto_approve_members} onChange={v => setSettings('auto_approve_members', v)} disabled={isDisabled} />
              </div>
              <div className="max-w-xs">
                <Field label="Máx. Visitas de Invitado" value={formData?.settings?.max_guest_visits?.toString()} onChange={v => setSettings('max_guest_visits', Number(v))} disabled={isDisabled} type="number" />
              </div>
            </SectionCard>
          </TabsContent>
            ...
            <SectionCard>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl border border-white/[0.08] shrink-0 flex items-center justify-center overflow-hidden"
                    style={{ background: formData.logoUrl ? `url(${getImageUrl(formData.logoUrl)}) center/cover no-repeat` : 'rgba(172,230,0,0.06)', color: A }}>
                    {!formData.logoUrl && <ImageIcon className="w-5 h-5 opacity-30" />}
                  </div>
                  <div className="flex-1">
                    <label className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border border-dashed text-xs font-semibold cursor-pointer transition-all w-fit ${isDisabled ? 'border-white/[0.06] text-white/25 cursor-not-allowed' : 'border-[#ace600]/25 text-[#ace600] hover:bg-[#ace600]/[0.05]'}`}>
                      {uploadingLogo ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
                      {formData.logoUrl ? 'Cambiar logo' : 'Subir logo'}
                      <input type="file" accept="image/*" className="hidden" disabled={isDisabled || uploadingLogo} onChange={handleLogoUpload} />
                    </label>
                    {formData.logoUrl && !isDisabled && (
                      <button type="button" onClick={() => set('logoUrl', '')} className="mt-1 text-[11px] text-red-400 hover:text-red-300 transition-colors">
                        Eliminar
                      </button>
                    )}
              ...
              <div>
                ...
                <div className="relative w-full h-24 rounded-xl border border-dashed overflow-hidden flex items-center justify-center transition-all"
                  style={{ background: formData.bannerImage ? `url(${formData.bannerImage}) center/cover no-repeat` : 'rgba(255,255,255,0.02)', borderColor: isDisabled ? 'rgba(255,255,255,0.06)' : 'rgba(172,230,0,0.2)' }}>
                  ...
                  {!isDisabled && (
                    <label className="absolute inset-0 cursor-pointer">
                      <input type="file" accept="image/*" className="hidden" disabled={isDisabled || uploadingBanner} onChange={handleBannerUpload} />
                    </label>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {formData.photos.map((url, i) => (
                    <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-white/[0.08] shrink-0">
                      ...
                      {!isDisabled && (
                        <button type="button" onClick={() => set('photos', formData.photos.filter((_, j) => j !== i))}
                  ...
                  {!isDisabled && (
                    <label className="w-20 h-20 rounded-xl border border-dashed flex flex-col items-center justify-center cursor-pointer shrink-0 transition-all hover:bg-[#ace600]/[0.04]"
                      style={{ borderColor: 'rgba(172,230,0,0.2)' }}>
                      {uploadingPhotos ? <Loader2 className="w-4 h-4 animate-spin" style={{ color: A }} /> : <Plus className="w-4 h-4" style={{ color: A }} />}
                      <span className="text-[10px] font-semibold mt-0.5" style={{ color: A }}>Agregar</span>
                      <input type="file" accept="image/*" multiple className="hidden" disabled={isDisabled || uploadingPhotos} onChange={handlePhotosUpload} />
                    </label>
                  )}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <Field label="Facebook" value={formData.socialMedia.facebook || ''} onChange={v => setSocialMedia('facebook', v)} disabled={isDisabled} placeholder="https://facebook.com/…" />
                  <Field label="Instagram" value={formData.socialMedia.instagram || ''} onChange={v => setSocialMedia('instagram', v)} disabled={isDisabled} placeholder="https://instagram.com/…" />
                  <Field label="Twitter / X" value={formData.socialMedia.twitter || ''} onChange={v => setSocialMedia('twitter', v)} disabled={isDisabled} placeholder="https://twitter.com/…" />
                </div>
              </div>
            </SectionCard>
          </TabsContent>
          ...
          <TabsContent value="owner" className="mt-4">
            <SectionCard>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Nombre del Propietario" value={formData.owner.name} onChange={v => setOwner('name', v)} disabled={isDisabled} />
                <Field label="Email del Propietario" value={formData.owner.email} onChange={v => setOwner('email', v)} disabled={isDisabled} type="email" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="ID del Propietario" value={formData.owner.id} onChange={v => setOwner('id', v)} disabled={isDisabled} />
                <Field label="Foto (URL)" value={formData.owner.profilePhoto || ''} onChange={v => setOwner('profilePhoto', v)} disabled={isDisabled} />
              </div>
              {(formData.owner.name || formData.owner.email) && (
                <div className="flex items-center gap-3 p-4 bg-white/[0.02] border border-white/[0.05] rounded-xl">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 overflow-hidden border border-white/[0.08]"
                    style={{ background: formData.owner.profilePhoto ? `url(${formData.owner.profilePhoto}) center/cover no-repeat` : 'rgba(172,230,0,0.08)', color: A }}>
                    {!formData.owner.profilePhoto && formData.owner.name?.[0]?.toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white/80">{formData.owner.name || '—'}</p>
          ...
          <TabsContent value="hours" className="mt-4">
            <SectionCard>
              <div className="hidden sm:grid grid-cols-[1fr_160px_160px] pb-3 border-b border-white/[0.05]">
                {['Día', 'Apertura', 'Cierre'].map(h => <span key={h} className={labelCls}>{h}</span>)}
              </div>
              {hours ? (
                <div className="flex flex-col">
                  {DAYS.map(({ key, label, short }) => {
                    ...
                    return (
                      <div key={key} className="grid grid-cols-1 sm:grid-cols-[1fr_160px_160px] items-center gap-3 py-3 border-b border-white/[0.04] last:border-0">
                        <div className="flex items-center gap-3">
                          <span className="w-9 h-9 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0 transition-all"
                            style={{ background: active ? 'rgba(172,230,0,0.08)' : 'rgba(255,255,255,0.03)', color: active ? A : 'rgba(255,255,255,0.2)', border: `1px solid ${active ? 'rgba(172,230,0,0.18)' : 'rgba(255,255,255,0.05)'}` }}>
                            {short}
                          </span>
                        ...
                        <div>
                          ...
                          <input type="time" value={hours[key]?.open || ''} disabled={isDisabled} onChange={e => setHour(key, 'open', e.target.value)} className={inputCls} />
                        </div>
                        <div>
                          <label className={`${labelCls} sm:hidden`}>Cierre</label>
                          <input type="time" value={hours[key]?.close || ''} disabled={isDisabled} onChange={e => setHour(key, 'close', e.target.value)} className={inputCls} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="flex flex-col items-center py-12 text-center">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-3 border" style={{ background:'rgba(172,230,0,0.06)', borderColor:'rgba(172,230,0,0.12)', color:A }}>
                    <Clock className="w-5 h-5" />
                  </div>
                  <p className="text-sm font-semibold text-white/50">Sin horarios configurados</p>
                  <p className="text-xs text-white/25 mt-1">Activa la edición para agregar horarios</p>
                </div>
              )}
            </SectionCard>
          </TabsContent>
