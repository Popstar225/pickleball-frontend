  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="flex flex-col items-center gap-5">
        ...
        <p className="text-xs font-bold text-slate-300 tracking-[0.25em] uppercase">Cargando micrositio…</p>
      </div>
    </div>
  );
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            ...
            <div className="hidden md:flex items-center">
              {navItems.map(item => (
                <a key={item.id} href={`#${item.id}`}
                  className="relative px-3 py-4 text-[10px] font-black uppercase tracking-widest text-slate-200 hover:text-slate-900 transition-colors group">
                  {item.label}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                className="md:hidden p-2 rounded-lg text-slate-300 hover:text-slate-900 hover:bg-slate-100 transition-all"
                ...
                <div className="py-2 grid grid-cols-2 gap-1">
                  {navItems.map(item => (
                    <a key={item.id} href={`#${item.id}`} onClick={() => setMobileNavOpen(false)}
                      className="px-3 py-2.5 text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-slate-900 hover:bg-slate-50 rounded-lg transition-colors">
                      {item.label}
                    </a>
                  ))}
                </div>
              ...
              {microsite.sponsors.map((sp, i) => (
                ...
                >
                  {sp.logo_url
                    ...
                    : <span className="font-black text-sm text-slate-200 group-hover:text-slate-700 transition-colors tracking-wide"
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5 mb-2">
                          ...
                          {ann.is_pinned && (
                            <span className="px-2 py-0.5 text-[9px] font-black rounded-full uppercase tracking-widest bg-slate-100 text-slate-300">
                              Fijado
                            </span>
                          )}
                          {ann.priority === 'high' && (
                        ...
                        <p className="text-slate-200 text-sm leading-relaxed line-clamp-2">{ann.content}</p>
                      ...
                      <ArrowRight className="w-4 h-4 text-slate-200 group-hover:text-slate-200 group-hover:translate-x-0.5 transition-all shrink-0 mt-1" />
                    </div>
                  </div>
                </motion.article>
              ))}
              ...
              {coaches.map((c, i) => (
                ...
                >
                  ...
                  <div className="px-5 pb-5 -mt-1">
                    ...
                    >{c.full_name}</h3>
                    {(c.city || c.state) && (
                      <p className="text-sm text-slate-200 flex items-center gap-1.5 font-medium mt-1.5">
                        <MapPin className="w-3.5 h-3.5 shrink-0" style={{ color: tc }} />
                        {[c.city, c.state].filter(Boolean).join(', ')}
                      </p>
                    )}
                    {c.bio && <p className="text-sm text-slate-300 line-clamp-2 leading-relaxed mt-2">{c.bio}</p>}
                  </div>
                </motion.div>
              ))}
            </div>
                ...
                <div className="relative p-7 md:p-9">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    {nextTournament.tournament_type && (
                      <span className="px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-white/70 text-slate-300">
                        {nextTournament.tournament_type}
                      </span>
                    )}
                  </div>
                  ...
                  >
                    <div className="flex flex-col sm:flex-row">
                      ...
                      >
                        ...
                        {t.end_date && <span className="text-[10px] text-slate-200 sm:mt-0.5">– {formatDate(t.end_date)}</span>}
                      </div>
                      {/* Content */}
                      <div className="flex-1 p-4">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {isPast ? (
                              <span className="px-2.5 py-1 text-[9px] font-black rounded-full uppercase tracking-widest bg-slate-100 text-slate-200">
                                Finalizado
                              </span>
                            ) : isUpcoming ? (
                        ...
                        {t.tournament_type && (
                          <span className="inline-flex mt-2 px-2 py-0.5 text-[9px] font-black rounded-full uppercase tracking-widest bg-slate-100 text-slate-300">
                            {t.tournament_type}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      {court.is_available
                        ...
                        : <span className="flex items-center gap-1 text-[10px] font-black text-slate-200 uppercase tracking-wider shrink-0">
                            <XCircle className="w-3.5 h-3.5" /> No disponible
                          </span>
                      }
                    </div>
                    <p className="text-sm text-slate-200 font-medium capitalize mb-3">{court.court_type} · {court.surface}</p>
                    ...
                    {court.description && <p className="text-xs text-slate-200 mt-3 line-clamp-2 leading-relaxed">{court.description}</p>}
                  </div>
                </motion.div>
              ))}
            </div>
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-7 md:p-8 flex flex-col justify-center">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="px-2.5 py-0.5 text-[9px] font-black rounded-full uppercase tracking-widest bg-slate-100 text-slate-300">
                      Destacado
                    </span>
                  </div>
                  <h3
                  ...
                  {blogPosts[0].excerpt && (
                    <p className="text-slate-200 text-sm leading-relaxed line-clamp-3 mb-5">{blogPosts[0].excerpt}</p>
                  )}
                ...
                {blogPosts.slice(1).map((post, i) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      {post.excerpt && (
                        <p className="text-slate-200 text-xs mt-1.5 line-clamp-2 leading-relaxed">{post.excerpt}</p>
                      )}
                      <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
                        <span className="flex items-center gap-1 text-xs text-slate-300 font-medium">
                          <Calendar className="w-3 h-3" />{formatDate(post.published_at)}
                        </span>
                        <ArrowRight className="w-4 h-4 text-slate-200 group-hover:translate-x-0.5 group-hover:text-slate-300 transition-all" />
                      </div>
                    </div>
                  </motion.article>
                ))}
                ...
                <div className="space-y-4 relative">
                  {[
                    ...
                    <div key={item.label} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-200 mb-0.5">{item.label}</p>
                        {item.href
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-200 font-medium mt-1">{label}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {typeLabel && <p className="text-xs text-slate-300 font-medium">{typeLabel}</p>}
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
              {navItems.slice(0, 5).map(item => (
                <a key={item.id} href={`#${item.id}`}
                  className="text-[10px] text-slate-300 hover:text-slate-300 font-black uppercase tracking-widest transition-colors">
                  {item.label}
                </a>
              ))}
            </div>
...
function SectionHead({ icon: Icon, title, subtitle, tc }: {
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-200 text-sm font-medium ml-12">{subtitle}</p>
    </motion.div>
  );
}
