...
export default function ClubsManagement() {
  ...
  const loadClubsData = useCallback(() => {
    dispatch(
      ...
      setActionError('Error loading clubs');
    });
  }, [dispatch, pagination.page, pagination.limit]);

  // Update local pagination when Redux pagination changes
  useEffect(() => {
    if (reduxPagination) {
      setPagination((prev) => ({
        ...prev,
        total: reduxPagination.total,
        pages: reduxPagination.pages,
      }));
    }
  }, [reduxPagination]);

  // Sync clubs from Redux
  const clubs = useMemo(() => {
    return reduxClubs as any;
  }, [reduxClubs]);

  // Get unique states for filter
  const uniqueStates = useMemo((): string[] => {
    const states = (clubs as Club[]).map((c: Club) => c.state || '').filter(Boolean);
  ...
  useEffect(() => {
    ...
    // Calculate stats from Redux clubs
    setStats({
      total: clubs.length,
  ...
  // Action handlers
  const handleVerifyClub = async (clubId: string, currentStatus: boolean) => {
    ...
    try {
      ...
      setSuccessMessage(
        `Club ${!currentStatus ? 'verified' : 'verification revoked'} successfully`,
      );
      setSelectedClubId(null);
      setTimeout(() => setSuccessMessage(null), 3000);
      loadClubsData();
    } catch (error: any) {
      setActionError(error.message || 'Failed to update club verification status');
    } finally {
      setActionLoading(false);
    }
  };

  const handleActivateClub = async (clubId: string, currentStatus: boolean) => {
    setActionLoading(true);
    setActionError(null);
    try {
      await dispatch(
      ...
      setSuccessMessage(`Club ${!currentStatus ? 'activated' : 'deactivated'} successfully`);
      setSelectedClubId(null);
      setTimeout(() => setSuccessMessage(null), 3000);
      loadClubsData();
    } catch (error: any) {
      setActionError(error.message || 'Failed to update club status');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleDeleteClub = async (clubId: string) => {
    ...
    try {
      ...
      setActionError(error.message || 'Failed to delete club');
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <div className="space-y-6 p-6">
      ...
      {(actionError || reduxError) && (
        <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-red-300 mt-1">{actionError || reduxError}</p>
          </div>
          <button
            onClick={() => {
              setActionError(null);
      ...
      {successMessage && (
        <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-green-400">Éxito</p>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          >
                            ...
                            <DropdownMenuSeparator className="bg-slate-700" />
                            <DropdownMenuItem
                              onClick={() => handleVerifyClub(club.id, club.is_verified)}
                            ...
                            <DropdownMenuItem
                              onClick={() => handleActivateClub(club.id, club.is_active)}
                              disabled={actionLoading}
                              className="text-slate-300 focus:bg-slate-700 focus:text-white"
                            >
                              {club.is_active ? (
